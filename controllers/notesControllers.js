...
const updateANote = async (req, res) => {
  try {
    ...
    if (note.title || note.content || note.category) {
      ...
      const updated = await NotesModel.findByIdAndUpdate(id, note, {
        new: true,
      });
      res.status(200).json({
...
const archiveANote = async (req, res) => {
  const id =
    typeof req.params.id === "string" && req.params.id.length === 24
      ? req.params.id
      : false;
...
const getBookmarkedNotes = (req, res, next) => {
  if (res.locals.user.userType === "generalUser") {
    GeneralUser.find({
      _id: res.locals.user._id,
    })
...
const getArchivedNotes = (req, res, next) => {
  if (res.locals.user.userType === "generalUser") {
    GeneralUser.find({
      _id: res.locals.user._id,
    })
